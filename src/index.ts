    ...
    try {
        ...
        if (process.env.DISCORD_TOKEN) {
            if (!process.env.OSU_CLIENT || !process.env.OSU_SECRET) {
                if (config.compatibilityMode === true) {
                    console.log(colorLog.yellow, "[W] Osu Client/Secret is marked as missing but compatibility mode is enabled! Error might happend.");
                } else {
                    await osuCheck();
                }
            };
