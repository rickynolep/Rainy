...
export default { 
	...
	async execute(interaction: ChatInputCommandInteraction) {
        ...
        let character
        if (nekores.anime.title !== null) {character = `${nekores.anime.character} (${nekores.anime.title || 'No title'})`} 
        else character = ''
        const desc = (
            `Artist: [${nekores.attribution.artist.username}](${nekores.attribution.artist.profile})\n` +
            `${character}` +
