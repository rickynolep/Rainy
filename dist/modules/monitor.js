...
export default {
    ...
    async execute(message) {
        try {
            ...
            const autoRespondList = getConfig().autoRespond.map((w) => w.toLowerCase());
            ...
            if (autoRespondList.some((w) => msgLower.includes(w)) && !message.content.trim().startsWith(getConfig().alwaysIgnoreSymbol)) {
                decrementCooldown();
            ...
            else if ((getConfig().alwaysRespond.includes(channelId) && !message.content.trim().startsWith(getConfig().alwaysIgnoreSymbol)) ||
