...
let lastAdminDisabled = [];
let lastUserDisabled = [];
let firstCheck;
function setSlash(hierarchy, filepath, set) {
...
function getDisabledCommands(commandsDir) {
    const result = [];
...
export async function reloadSlash() {
    ...
    if (getConfig().enableAfk === false) {
        setSlash("user", 's-afk', false);
    }
    else {
        setSlash("user", 's-afk', true);
    }
    ;
    // if (getConfig().enableCatbox === false) {setSlash("user", 's-catbox', false)} else {setSlash("user", 's-catbox', true)};
    if (getConfig().enableNeko === false) {
        setSlash("user", 's-neko', false);
    }
    else {
        setSlash("user", 's-neko', true);
    }
    ;
    if (getConfig().enableOsu === false) {
        setSlash("user", 's-osu', false);
    }
    else {
        setSlash("user", 's-osu', true);
    }
    ;
    if (getConfig().enablePing === false) {
        setSlash("user", 's-ping', false);
    }
    else {
        setSlash("user", 's-ping', true);
    }
    ;
    if (JSON.stringify(currentAdminDisabled) !== JSON.stringify(lastAdminDisabled) ||
        ...
        if (getConfig().verbose) {
            console.log(dim('[I] Checking application (/) commands...'));
        }
        ;
